...
router.post('/_last_tortoise_key', (req, res) => {
  ...
  log('\n #1 HTTP <== from Turtle requesting checkpoint from last sync session');
  ...
  .then(lastKey => {
    ...
    log('\n #2 HTTP ==> to Turtle with last key');
...
router.post('/_missing_rev_ids', (req, res) => {
  log(`\n #3 HTTP <== from Turtle with (${req.body.metaDocs.length}) changed revision trees`);
  tortoiseDB.syncFromSession.findAllMissingLeafNodes(req.body.metaDocs)
    .then(missingRevIds => {
      //log('\n Merge revision trees and list all missing records');
      log(`\n #4 HTTP ==> to Turtle requesting (${missingRevIds.length}) missing records`);
...
router.post('/_insert_docs', (req, res) => {
  log('\n #5 HTTP <== from Turtle with missing records and updated sync history');
  ...
  .then(() => log('\n #6 HTTP ==> to Turtle with confirmation of complete sync'))
